...
class CommerceWebformOrderHandler extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    if ($this->shouldBeExecuted($webform_submission)) {
      try {
        ...
        $this->orderItem = $order_item_storage->create([
        ...
        $this->cartManager->addOrderItem($this->cart, $this->orderItem);
        $this->cart->save();
  ...
  public function confirmForm(array &$form, FormStateInterface $form_state, WebformSubmissionInterface $webform_submission) {
    if ($this->configuration['checkout']['redirect'] && $this->shouldBeExecuted($webform_submission)) {
      try {
        $url = Url::fromRoute(
   ...
   */
  protected function shouldBeExecuted(WebformSubmissionInterface $webform_submission) {
    $state = $webform_submission->getWebform()->getSetting('results_disabled') ? WebformSubmissionInterface::STATE_COMPLETED : $webform_submission->getState();

    return $this->configuration['states'] && in_array($state, $this->configuration['states']);
  }
